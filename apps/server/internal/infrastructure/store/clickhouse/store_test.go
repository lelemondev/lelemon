...
// getTestDSN returns a ClickHouse connection string for testing
// Set CLICKHOUSE_TEST_URL env var or it will skip the test
func getTestDSN(t *testing.T) string {
	dsn := os.Getenv("CLICKHOUSE_TEST_URL")
	if dsn == "" {
		t.Skip("CLICKHOUSE_TEST_URL not set, skipping ClickHouse tests")
	}
	return dsn
}
...
func setupTestStore(t *testing.T) *Store {
	...
	// Clean up tables for fresh test (ClickHouse style)
	store.conn.Exec(ctx, "TRUNCATE TABLE spans")
	store.conn.Exec(ctx, "TRUNCATE TABLE traces")
	store.conn.Exec(ctx, "TRUNCATE TABLE projects")
	store.conn.Exec(ctx, "TRUNCATE TABLE users")
...
func TestClickHouseUserOperations(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
	...
	t.Run("update user", func(t *testing.T) {
		...
		// Wait a bit for ReplacingMergeTree to merge
...
func TestClickHouseProjectOperations(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
...
func TestClickHouseTraceOperations(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
...
func TestClickHouseSpanOperations(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
...
func TestClickHouseAnalytics(t *testing.T) {
	...
	t.Run("GetUsageTimeSeries hourly", func(t *testing.T) {
		opts := entity.TimeSeriesOpts{
			Period: entity.Period{
				From: time.Now().Add(-24 * time.Hour),
				To:   time.Now().Add(24 * time.Hour),
...
func TestClickHouseSessions(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
